...
nparACT_calculate_interval_mean = function(data_location, interval_missing_thres, day_missing_thres, k){
  ...
  for (i in 0:floor(timeOverall/timeRow)){
  ...
  #Windsorizer
  print("QUANTILE?")
  #print(unname(quantile(data_interval$Activity, na.rm = TRUE)[3]))
  ...
  if(TRUE){
    ...
    for (k in 0:(floor(nrow(data_interval)/interval_day)-1)){
      ...
      print((k*interval_day+1))
      print("beginning")
      print(((k+1)*interval_day))
      print("end")
...
print_summary = function(final_data, IS, IV, IV2, data_interval, k){
  print("-------------------------------------SUMMARY-----------------------------------------------")
  print(c("IS:", IS))
  print(c("IV:", IV))
